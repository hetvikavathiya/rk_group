...
class Account extends CI_Controller {
    ...
    function __construct()
	{
		...
		if ($this->session->userdata('loginstatus') != 2 || $this->session->userdata('admin_id') == "" && ($this->session->userdata('user_type') != "employee") || (!in_array("Account Opening", $array_permission))) {
	...
	public function open($param=""){
		...
		$this->session->set_flashdata('flash_message', $message);
        redirect(base_url('employee/account/profile/'.$customer_id), 'refresh');
	}
