...
class Loan extends CI_Controller
{
	...
	function __construct()
	{
		...
		if ($this->session->userdata('loginstatus') != 2 || $this->session->userdata('admin_id') == "" && ($this->session->userdata('user_type') != "employee") || (!in_array("Loan", $array_permission))) {
	...
	public function create($customer_id)
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->session->set_flashdata('flash_message', $message);
			redirect(base_url('employee/loan/profile/' . $customer_id), 'refresh');
		} else {
	...
	public function get_loan_details()
	{
		...
		return $this->load->view("employee/loan_details", compact("loan", "loan_details","late_fee_percentage","payment_mode","customer_id"));
	}
