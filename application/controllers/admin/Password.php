...
class Password extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        if ($this->session->userdata('loginstatus')!= 1 || $this->session->userdata('admin_id')=="" && ($this->session->userdata('user_type') !="Admin" || $this->session->userdata('user_type') !="User")) {            $message = array('message'=>"Your Session has Been Expired.!!", 'class'=>'danger');
	...
	public function change(){
	    ...
	    $this->form_validation->set_rules('re_password', 're_password', 'trim|required');
	     if($this->form_validation->run()==false) {
            ...
            $this->session->set_flashdata('flash_message',$message);
            redirect(base_url("admin/password"),'refresh');
        }
        else {
            ...
            if($old_password == $db_password){
                ...
                $pass_update = $this->db->update('admin',array('password'=>sha1($re_password)));
                ...
                $this->session->set_flashdata('flash_message',$message);
                redirect(site_url('admin/password'),'refresh');
            }
