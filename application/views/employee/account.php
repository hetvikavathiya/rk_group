			...
			<div class="row">
				...
				<?php }else if(!empty($row_data) && $row_data['account_status'] == 0){ ?>
				    <div class="col-lg-12">
						...
						<div class="form-group">
							<a href="<?=base_url('employee/customer_report/edit/');?><?= $customer_id; ?>/1" class="btn btn-primary px-5">Upload Document</a>
						</div>
					</div>
				<?php }else if(!empty($row_data)){ ?>
				 	<span>Account Already Exists &nbsp;<a href="<?=base_url('employee/credit_debit/profile/');?><?php if(!empty($customer_id)){echo $customer_id;}?>" class="btn btn-primary px-5">View Account</a></span>
			<?php }?>
			</div>
		</div>
	</div>
	...
	<div class="modal-dialog modal-xl">
		<div class="modal-content" style="background-image:url(<?= base_url('assets/'); ?>images/bg-themes/1.png);">
			...
			<div class="modal-body">
				<form method="post" action="<?php echo base_url('employee/account/open/' . $customer_id); ?>"  enctype="multipart/form-data">
					...
					<div class="row">
						...
						<div class="col-lg-4">
							<div class="form-group">
								...
								<input type="number" step="any" class="form-control" id="input-33" name="amount" value="<?=$open_account_charge['open_account_charge'];?>" required>
							</div>
						</div>
						<div class="col-lg-4">
    						<div class="form-group">
...
<script>
    $(document).ready(function() {
		...
		$(document).on('change', '.permission', function() {
			...
			var url = '<?= base_url("employee/account/profile/"); ?>';
