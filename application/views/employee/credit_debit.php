	...
	<div class="col-lg-12">
		<div class="card p-3">
			...
			<div class="row">
				...
				<div class="col-lg-3 col-md-3 col-sm-4 col-4">
				    <a href="<?=base_url('employee/credit_debit/close/');?><?php if(!empty($customer_id)){echo $customer_id;}?>" class="btn btn-danger"  onclick="return confirm('Are you sure?')">Close Bank Account</a>
				</div>
				<?php }?>
  ...
  <div class="modal-dialog">
    <div class="modal-content" style="background-image:url(<?= base_url('assets/'); ?>images/bg-themes/1.png);">
      ...
      <div class="modal-body">
        	<form method="post" action="<?=base_url('employee/credit_debit/credit/'.$customer_id);?>"  enctype="multipart/form-data">
...
<div class="modal fade pay_amount_modal" id="staticBackdrop2" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content" style="background-image:url(<?= base_url('assets/'); ?>images/bg-themes/1.png);">
      ...
      <div class="modal-body">
        	<form method="post" action="<?=base_url('employee/credit_debit/debit/'.$customer_id);?>"  enctype="multipart/form-data">
				<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>" />
				<input type="hidden" value="<?= $customer_id;?>" name="customer_id">
				<input type="hidden" value="" name="loan_emi_details_id" id="loan_emi_details_id">
				<div class="row">
...
<script>
    $(document).ready(function() {
        var userDataTable = $('#example1').DataTable({
			...
			'serverMethod': 'get',
			'ajax': {
				'url': '<?php echo base_url(); ?>employee/credit_debit/report',
				'data': function(data) {
		...
		$(document).on('change', '.permission', function() {
			...
			var url = '<?= base_url("employee/credit_debit/profile/"); ?>';
