 				...
 				<div class="col-lg-6 col-md-6 col-sm-12 col-12" id='profile-hide'>
 					<div class="card profile-card-2">
 						...
 						<div class="card-body pt-5">
 							<img src="<?= base_url('upload/'); ?><?php if (!empty($row_data['photo'])) {
																		echo $row_data['photo'];
																	} else {
																		echo 'avatar.png';
 ...
 <div class="row mt-3 loan_report">
 	<div class="col-lg-12">
 		<div class="card">
 			...
 			<input type="hidden" value="<?php if (!empty($customer_id)) {
												echo $customer_id;
											} ?>" id="customer_id">
 			<div class="card-body">
 	...
 	<div class="modal-dialog modal-xl">
 		<div class="modal-content" style="background-image:url(<?= base_url('assets/'); ?>images/bg-themes/1.png);">
 			...
 			<div class="modal-body">
 				<form method="post" action="<?php echo base_url('admin/loan/create/' . $customer_id); ?>">
 					...
 					<div class="row">
 						...
 						<div class="col-lg-4">
 							<label for="select121">Loan Type<span class="text-danger">*</span></label>
 							<select class="form-select" name="loan_type_id" id="loan_type_id">
 								<option>Select Loan type</option>
 								<?php
									$loan_type = $this->db->get('loan_type')->result();
									foreach ($loan_type as $value) {
									?>
 									<option value="<?= $value->id; ?>" <?php if (isset($update_data) && $value->id == $update_data['loan_type_id']) {
																			echo 'selected';
																		} ?>><?= $value->name; ?></option>
 								<?php } ?>
 							</select>
 ...
 <script>
 	$(document).ready(function() {
 		...
 		$(document).on('change', '.permission', function() {
 			...
 			var url = '<?= base_url("admin/loan/profile/"); ?>';
 		...
 		$(document).on("click", ".loan_details", function() {
 			...
 			$.ajax({
 				url: "<?= base_url("admin/loan/get_loan_details"); ?>",
 				type: "POST",
 				timeout: 5000,
 				processData: true,
 				data: {
 		...
 		var loanData = $('#loan_report').DataTable({
 			...
 			'serverMethod': 'post',
 			'ajax': {
 				'url': '<?php echo base_url(); ?>admin/loan/report',
 				'data': {
 					[csrf_name]: csrf_token,
